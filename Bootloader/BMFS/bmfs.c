        ...
        } else // Opened ok, is it a valid BMFS disk?
        {
                    ...
                    0) // Is it a BMFS formatted disk?
                {
                        if (strcasecmp(s_format, command) == 0) {
                                ...
                                printf("Error: Not a valid BMFS drive "
                                       "(Disk is not BMFS formatted).\n");
                        }
                        fclose(disk);
                        return 0;
                }
...
int initialize(char *diskname, char *size, char *mbr, char *boot, char *kernel)
{
        ...
        for (i = 0; size[i] != '\0' && ret == 0; ++i) {
                ...
                {
                        printf("Error: A numeric disk size must be "
                               "specified\n");
                        ret = 1;
                } else {
                        switch (toupper(ch)) {
                        ...
                        case 'P':
                                ...
                                break;
                        default:
                                printf("Error: Invalid disk size string: "
                                       "'%s'\n",
                                       size);
                                ret = 1;
                                break;
        ...
        if (ret == 0) {
                ...
                while (writeSize < diskSize) {
                        ...
                        percent /= diskSize;
                        percent *= 100;
                        printf("Formatting disk: %llu of %llu bytes "
                               "(%.0f%%)...\r",
...
void create(char *filename, unsigned long long maxsize)
{
        ...
        if (findfile(filename, &tempentry, &slot) == 0) {
                ...
                int num_used_entries =
                        0; // how many entries of Directory are either used or
                // deleted
                ...
                for (tint = 0; tint < num_used_entries + 1; tint++) {
                        ...
                        if (tint == num_used_entries ||
                            pEntry->FileName[0] == 0x01)
                                this_file_start = num_blocks -
                                                  1; // index of the last block
                        else
                ...
                if (first_free_entry == num_used_entries &&
                    num_used_entries + 1 < 64) {
                        ...
                        // exists
                        pEntry = (struct BMFSEntry *)(Directory +
                                                      (num_used_entries + 1) *
                                                              64);
        ...
        if (0 == findfile(filename, &tempentry, &slot)) {
                ...
                if ((tfile = fopen(tempentry.FileName, "wb")) == NULL) {
                        ...
                        fseek(disk, tempentry.StartingBlock * 2097152,
                              SEEK_SET); // Skip to the starting block in the
                                // disk
...
void write(char *filename)
{
        ...
        if (0 == findfile(filename, &tempentry, &slot)) {
                printf("Error: File not found in BMFS. "
                       "A file entry must first be created.\n");
        } else {
