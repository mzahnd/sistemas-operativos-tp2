...
void highlightCurrent(int startX, int startY, double scl) {
  drawRect(round(startX + currentX * scl), round(startY + currentY * scl), round(scl), round(scl), PALE_PINK);
}
...
void drawCells(int startX, int startY, double scl) {
  ...
  for (int i = 0; i < 9; i++) {
    for (int j = 0; j < 9; j++) {
      ...
      drawRect(round(startX + j * scl), round(startY + i * scl), round(scl), round(scl), color);
      count++;
    }
  }
}
...
void drawNumbers(int startX, int startY, double scl) {
  ...
  for (int i = 0; i < 9; i++) {
    for (int j = 0; j < 9; j++) {
      ...
      if (fixed[i][j] == 1) {
        color = BLACK;
      } else if (!(checkHorizontal(i, j) && checkVertical(i, j) && checkKernel(i, j))) {
        color = RED;
      }
      if (board[i][j] != 0) {
        char ch = board[i][j] + '0';
        drawString(round(startX + j*scl + scl/2 - BASE_CHAR_WIDTH), round(startY + i*scl + scl/2 - BASE_CHAR_HEIGHT), &ch, 1, color, WHITE, 2, 1);
      }
    }
  }
}
...
void draw_sudoku() {
  drawRect(SCREEN_WIDTH/2, SCREEN_HEIGHT/2, SCREEN_WIDTH/2, SCREEN_HEIGHT/2, WHITE);
  ...
  if (sudokuWin && sudokuTotal == 9*9) {
    drawString(SCREEN_WIDTH/2 + SCREEN_WIDTH/4 - round(4*BASE_CHAR_WIDTH*3.5), SCREEN_HEIGHT/2 + SCREEN_HEIGHT/4 - 2*BASE_CHAR_HEIGHT, "YOU WIN", strlen("YOU WIN"), RED, BLACK, 4, 0);
  }
}
...
int checkKernel(int i, int j) {
  ...
  for (int ii = 0; ii < 3; ii++) {
    for (int jj = 0; jj < 3; jj++) {
      ...
      if (board[cordI][cordJ] == board[i][j] && (cordI != i || cordJ != j)) {
        sudokuWin = 0;
        return 0;
      }
    }
