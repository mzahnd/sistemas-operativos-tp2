...
void draw_word()
{
        ...
        str[i] = 0;
        drawString(SCREEN_WIDTH / 4 -
                           round(strlen(str) / 2.0 * 3 * BASE_CHAR_WIDTH),
                   SCREEN_HEIGHT / 2 + SCREEN_HEIGHT / 4, str, strlen(str),
                   BLACK, BLACK, 3, 1);
}

void draw_game_over()
{
        drawString(SCREEN_WIDTH / 4 - round(strlen("Game Over") / 2.0 * 4 *
                                            BASE_CHAR_WIDTH),
                   SCREEN_HEIGHT / 2 + SCREEN_HEIGHT / 4, "Game Over",
                   strlen("Game Over"), RED, BLACK, 4, 1);
}
void draw_win()
{
        drawString(SCREEN_WIDTH / 4 -
                           round(strlen("You Win") / 2.0 * 4 * BASE_CHAR_WIDTH),
                   SCREEN_HEIGHT / 2 + SCREEN_HEIGHT / 4 + BASE_CHAR_HEIGHT * 4,
                   "You Win", strlen("You Win"), RED, BLACK, 4, 1);
}

void draw_background()
{
        drawRect(0, SCREEN_HEIGHT / 2, SCREEN_WIDTH / 2, SCREEN_HEIGHT / 2,
                 PALE_PINK);
}
...
void draw_lives()
{
        ...
        drawString(0, SCREEN_HEIGHT / 2 + BASE_CHAR_HEIGHT * 3, lives,
                   strlen(lives), BLACK, BLACK, 3, 1);
}
