...
static void drawVerticalLine(uint64_t x, uint64_t yStart, uint64_t yEnd,
                             uint64_t col);

struct vbe_mode_info_structure {
        uint16_t attributes; // deprecated, only bit 7 should be of interest to
                // you, and it indicates the mode supports a
                // linear frame buffer.
...
struct vbe_mode_info_structure *screenData =
        (struct vbe_mode_info_structure *)0x0000000000005C00;
...
void drawLine(uint64_t xStart, uint64_t yStart, uint64_t xEnd, uint64_t yEnd,
              uint64_t col)
{
        if (!legalCoordinates(xStart, yStart) || !legalCoordinates(xEnd, yEnd))
                return;
        if (xStart == xEnd) {
                ...
                for (uint64_t i = xStart; i <= xEnd; i++) {
                        ...
                        uint8_t *pos =
                                (uint8_t *)(FRAME_BUFFER + (y * WIDTH + i) * 3);
                        *pos = b;
                        pos++;
                        *pos = g;
                        pos++;
              ...
              uint8_t alphaBackground)
{
        ...
        for (uint8_t i = 0; i < CHAR_HEIGHT; i++) {
                for (uint8_t j = 0; j < CHAR_WIDTH; j++) {
                        ...
                        if (bitIsPresent) {
                                drawRectangle(aux_x, aux_y, fontSize, fontSize,
                                              fontColor);
                ...
                uint64_t height, uint64_t size)
{
        for (int i = 0; i < height * size; i++) {
                for (int j = 0; j < width * size; j++) {
                        if (!legalCoordinates(x + j, y + i))
                                continue;
                        if (mat[(i / size) * width + (j / size)] == 0x01000000)
                                continue;
...
static void drawVerticalLine(uint64_t x, uint64_t yStart, uint64_t yEnd,
                             uint64_t col)
{
        if (!legalCoordinates(x, yStart) || !legalCoordinates(x, yEnd))
                return;
        uint8_t b = col & 0x0000FF;
        uint8_t g = (col >> 8) & 0x0000FF;
        uint8_t r = (col >> 16) & 0x0000FF;
